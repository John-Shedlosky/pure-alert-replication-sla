...
from .protection_report import (
    ...
    aggregate_fa_volume_rows, _load_recent_comments,
...
pair next to color-bearing columns sorts by status.
  ...
  Comments column (read-only)
      The right-most column shows a free-text comment plus a "Last
      updated" timestamp for each (array, volume) pair. Both values
      are read from a JSON sidecar at report-generation time:

          reports/protection/comments_active.json

      The rendered HTML is intentionally read-only — there is no in-
      browser editor, no Save / Load buttons, and no autosave. To
      change a comment, edit comments_active.json directly and click
      "Run Daily Report" again to regenerate the HTML.

      File format (UTF-8 JSON):

          {
            "generated": "2026-05-03 09:15:00",
            "comments": [
              {
                "array":        "flasharray-prod",
                "volume":       "vol-app-01",
                "comment":      "Owner: App Team. Tier 1 SLA.",
                "last_updated": "2026-05-01 14:22:10"
              },
              ...
            ]
          }

      The loader uses the most recently modified comments_*.json file
      under reports/protection/ as its source, so older snapshots can
      be kept alongside the active file without affecting the report.
      Rows whose (array, volume) pair has no entry in the JSON render
      as an em-dash in both the comment and "Last updated" cells.
