...
* the exceptions.json discovery/load/save helpers and the HTML post-
  processor that paints per-row Config Drift Exception cells
...
def aggregate_fb_filesystem_rows(per_array):
    ...
    # policies (Policy column is '/'-delimited). Outbound replica-link
    # rows additionally seed two side maps:
    #   replica_targets[(arr, fs)] -> ordered list of Remote File System
    #     names for outbound replica-links from this filesystem. Rendered
    #     under the "Replica File System" column.
    #   remote_fs_exclude -> set of (remote_array, remote_file_system)
    #     pairs that represent destination-side replicas. Their entries
    #     in `purefs list` on the destination FB are suppressed from the
    #     row list because they are not standalone filesystems but the
    #     receiving end of an outbound link.
    repl_dests        = {}
    repl_policies     = {}
    replica_targets   = {}
    remote_fs_exclude = set()
    for arr, data in per_array.items():
        if data.get('platform') != 'FB':
            continue
        for link in data.get('fb_fs_replica_links', []):
            ...
            for _p in link.get('policies', []):
                if _p and _p not in _plst:
                    _plst.append(_p)
            # Outbound link: the source side owns the data, the remote
            # filesystem is a replica. Record the replica name for the
            # source row's "Replica File System" cell and add the remote
            # (array, fs) tuple to the exclusion set so the destination
            # FB doesn't render its own row for the same data.
            if (link.get('direction') or '').strip().lower() == 'outbound':
                rfs = link.get('remote_file_system', '')
                if remote and rfs:
                    _tlst = replica_targets.setdefault(_key, [])
                    if rfs not in _tlst:
                        _tlst.append(rfs)
                    remote_fs_exclude.add((remote, rfs))
    ...
    # filesystems the array contributes no rows. Destination-side
    # replicas are suppressed via remote_fs_exclude so they don't
    # appear as standalone rows on the receiving FB.
    rows = []
    for arr, data in per_array.items():
        if data.get('platform') != 'FB':
            continue
        for fs in data.get('fb_filesystems', []):
            ...
            if not fs_name:
                continue
            if (arr, fs_name) in remote_fs_exclude:
                continue
            ...
            r_targets = list(replica_targets.get((arr, fs_name), []))
            ...
            rows.append({
                ...
                'replica_file_systems':     r_targets,
                'local_snaps':              local_n,
...
# Predefined set of admin-selectable Config Drift exception reasons. The
# GUI combobox is populated verbatim from this list and the HTML
# post-processor uses the first entry to detect "still breaking SLA"
# exceptions when shading row colors.
EXCEPTION_CHOICES = [
    'None - Breaking SLA',
    'Test',
    'Development',
    ...
    'Managed by Third-Party Tool',
    'Read-Only / Static Data',
    'Legal / E-Discovery Hold',
    'Active Maintenance / Migration',
    'Approved Risk Waiver',
    'None',
    'Custom',
]


def _exceptions_json_path():
    """Resolve the on-disk path for the exceptions.json sidecar.
    Sits alongside the generated HTML under reports/protection/ so the
    file lives next to the artifact it annotates.
    """
    try:
    ...
    return os.path.join(out_dir, 'exceptions.json')


def _load_exceptions():
    """Load exceptions.json into a dict keyed by 'ARRAY#NAME'.
    Returns an empty dict when the file is missing or unparseable so
    callers can treat first-run discovery and corrupted state the same.
    """
    path = _exceptions_json_path()
...
def _save_exceptions(data):
    """Persist the exceptions dict to exceptions.json (pretty-printed)."""
    path = _exceptions_json_path()
    with open(path, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2, sort_keys=True)


def _compute_row_color(local_sla_green, repl_sla_green,
                       has_local, has_remote, sm_on):
    """Initial color decision for a freshly discovered row.
    Green when both SLA cells pass, the row carries local + remote
    snapshots, and Safemode is Enabled; red otherwise.
    """
    if (local_sla_green and repl_sla_green and
            has_local and has_remote and sm_on):
        return 'green'
    return 'red'


def _discover_and_update_exceptions(row_metas):
    """Reconcile newly-discovered rows against exceptions.json.

    row_metas: iterable of dicts with keys
    ...
    * Otherwise (an acknowledged exception is set), leave the stored
    ...
    for meta in row_metas:
        key = meta['key']
        wb  = meta.get('would_be_color', 'red')
...
# Background tones applied to the first-column cell of each row based on
# the exceptions.json color verdict. Grey is the fallback when no color
...
def update_html_with_exceptions(html):
    """Apply exceptions.json data to a rendered protection-report HTML.

    Operates on the HTML string returned by build_protection_html. For
    each row marker:
    * <td class="cde-name" data-ckey="KEY">...</td> gains an inline
      background-color matching the JSON entry's color (default grey).
    * <td class="cde-cell" data-ckey="KEY"></td> is populated with the
      exception_reason and Last_Update from the JSON entry.

    Idempotent: tolerates already-overlaid markup so a second pass
    refreshes colors / reasons in place.
    ...
    def _name_repl(m):
        ...
        bg    = _CDE_BG.get(color, _CDE_BG['grey'])
        return (f'<td class="cde-name" data-ckey="{key}" '
                f'style="background:{bg};">')

    # Match both the un-styled marker and the already-overlaid form so
    # update_html_with_exceptions can run repeatedly on the same HTML.
    html = re.sub(
        r'<td class="cde-name" data-ckey="([^"]+)"(?:\s+style="[^"]*")?>',
        _name_repl, html)

    def _cell_repl(m):
        key  = m.group(1)
        rec  = data.get(key) or {}
        reas = rec.get('exception_reason', '') or 'None'
        upd  = rec.get('Last_Update', '') or ''
        inner = (
            f'<div class="cde-text">{_html.escape(reas)}</div>'
            f'<div class="cde-meta">'
            f'<span class="cde-meta-lbl">Last updated:</span> '
            f'<span class="cde-updated">'
            f'{_html.escape(upd) if upd else "&mdash;"}'
            f'</span>'
            f'</div>')
        return f'<td class="cde-cell" data-ckey="{key}">{inner}</td>'

    # Accept the empty marker or any prior overlay payload between the
    # opening td and its </td>, so repeated overlays replace cleanly.
    html = re.sub(
        r'<td class="cde-cell" data-ckey="([^"]+)">.*?</td>',
        _cell_repl, html, flags=re.DOTALL)
    return html
...
def build_protection_html(per_array, config):
    ...
    # Per-array platform map ('FA' / 'FB') used to stamp array_type on
    # each row's exceptions.json entry. Volumes and FA filesystems
    # always resolve to 'FA' regardless of this map; FB filesystems use
    # the lookup to verify the row's owning array.
    _platform_of = {a: (info.get('platform') or '')
                    for a, info in per_array.items()}

    # Row metadata captured at render time and handed to
    # _discover_and_update_exceptions after all three tables are built.
    # Each entry mirrors the schema used by exceptions.json.
    row_metas = []
    ...
    if not rows:
        ...
        for r in rows:
            ...
            # Config Drift Exceptions cell: emitted empty here and
            # populated by update_html_with_exceptions from
            # exceptions.json after row metadata has been reconciled.
            _local_green = (sla_snap_days > 0
                            and r.get('local_status') == 'protected'
                            and snap_ret_n >= sla_snap_days)
            _repl_green  = (sla_repl_days > 0
                            and r.get('repl_status') == 'protected'
                            and repl_ret_n >= sla_repl_days)
            _wb = _compute_row_color(_local_green, _repl_green,
                                     local_n > 0, rep_n > 0, sm_on)
            _ckey = f'{r["array"]}|FA-Volume|{vol_disp}'
            row_metas.append({
                'key':             _ckey,
                'array_name':      r['array'],
                'volume_name':     vol_disp,
                'array_type':      'FA-Volume',
                'would_be_color':  _wb})
            tr_html += (
                '<tr>'
                f'<td class="cde-name" data-ckey="{_ckey}">'
                f'{_html.escape(vol_disp)}</td>'
                ...
                f'<td class="cde-cell" data-ckey="{_ckey}"></td>'
                '</tr>\n')
        ...
        for r in rows_t2:
            ...
            def _t2_repl_sla():
                ...
                if repl_ret_n >= sla_repl_days:
                    return f'<td style="{_GREEN_T2}">{_CHECK_T2}</td>'
                return f'<td style="{_RED_T2}">Short Retention</td>'
            # Config Drift Exceptions: would-be color derived from the
            # same predicates the SLA cells use; cell rendered empty
            # here and filled by update_html_with_exceptions.
            _local_green = (sla_snap_days > 0 and bool(pols_list)
                            and loc_ret_n >= sla_snap_days)
            _repl_green  = (sla_repl_days > 0 and rep_eligible
                            and bool(pols_list)
                            and repl_ret_n >= sla_repl_days)
            _wb = _compute_row_color(_local_green, _repl_green,
                                     local_n > 0, rep_n > 0, sm_on)
            _ckey = f'{r["array"]}|FA-Filesystem|{dir_disp}'
            row_metas.append({
                'key':            _ckey,
                'array_name':     r['array'],
                'volume_name':    dir_disp,
                'array_type':     'FA-Filesystem',
                'would_be_color': _wb})
            tr_html_t2 += (
                '<tr>'
                f'<td class="cde-name" data-ckey="{_ckey}">'
    ...
    fb_fs_exports = {}
    # Per-(array, filesystem) multi-protocol metadata used by the
    # Protocols column. The same record is folded into each export's
    ...
    for _arr, _data in per_array.items():
        if _data.get('platform') != 'FB':
            continue
        for _mp in _data.get('fb_fs_multi_protocol', []):
            _nm = _mp.get('name', '')
            if _nm:
    ...
    # Per-(array, export_name) modal payload. One profile per export
    # name carries the filesystem's multi-protocol summary plus a list
    # of export rows that share the name (an export with both NFS and
    ...
    for _arr, _data in per_array.items():
        if _data.get('platform') != 'FB':
            continue
        _nfs_rules = _data.get('fb_policy_nfs_rules', []) or []
        _smb_rules = _data.get('fb_policy_smb_share_rules', []) or []
        for _exp in _data.get('fb_fs_exports', []) or []:
            _en = _exp.get('export_name') or _exp.get('name', '')
            if not _en:
                continue
            _fs = _exp.get('file_system', '')
            ...
            prof = fb_export_profiles.setdefault(_arr, {}).get(_en)
            if prof is None:
                _mp = fb_fs_mp.get((_arr, _fs), {})
                prof = {
                    'file_system': _fs,
                    'protocols': {
                        'protocols':            _mp.get('protocols', ''),
                        'access_control_style': _mp.get('access_control_style', ''),
                        'safeguard_acls':       _mp.get('safeguard_acls', '')},
                    'exports': []}
    ...
    tr_html_t3 = ""
    if not rows_t3:
        tr_html_t3 = ('<tr><td colspan="15" style="text-align:center;color:#888;">'
        ...
        for r in rows_t3:
            ...
            dests = (', '.join(_html.escape(d)
                     ...
                     if dests_ok else _DASH)
            # Replica File System column: destination-side filesystem
            # names captured by aggregate_fb_filesystem_rows from each
            # outbound replica-link's Remote File System column. The
            # destination FB's own row for the same filesystem has
            # already been suppressed by the aggregator's exclusion
            # set, so this cell is the only place the replica name
            # appears in Table 3.
            rfs_list = r.get('replica_file_systems') or []
            rfs_ok   = bool(rfs_list)
            rfs_cell = (', '.join(_html.escape(d) for d in rfs_list)
                        if rfs_ok else _DASH)
            local_n = int(r['local_snaps'])
            rep_n   = int(r['replicated_snaps'])
            ...
            # Replication policies are still tracked on the row for
            # SLA-M evaluation below, but the column itself was
            # dropped from Table 3 in favor of Replica File System.
            # Protocols column: comma-joined Protocols value from
            # purefs list --multi-protocol for this filesystem. Cells
            # render neutral (no green/red) so the column doesn't
            # compete with the explicit Safemode / SLA shading.
            _mp = fb_fs_mp.get((arr, fs_name), {})
            proto_str = (_mp.get('protocols') or '').strip()
            proto_cell = (_html.escape(proto_str) if proto_str else _DASH)
            # Exports cell: each name becomes a clickable link to the
            # FB-specific export modal (showFbExport) which renders
            # the Protocols mini-table, Export Info rows, and matched
            # NFS / SMB rule tables.
            exp_list = fb_fs_exports.get((arr, fs_name), [])
            exp_ok   = bool(exp_list)
            def _exp_link(e, _a=arr):
                return (f'<a href="#" class="pg-link" '
                        f'data-arr="{_html.escape(_a, quote=True)}" '
                        f'data-exp="{_html.escape(e, quote=True)}" '
                        f'onclick="showFbExport(this);return false;">'
                        f'{_html.escape(e)}</a>')
            exp_cell = (', '.join(_exp_link(e) for e in exp_list)
            ...
            # Config Drift Exceptions: would-be color uses the same
            # predicates as the SLA cells above. Cell rendered empty
            # and filled by update_html_with_exceptions.
            _local_green = (sla_snap_days > 0 and bool(pols_list)
                            and local_n > 0
                            and loc_ret_n >= sla_snap_days)
            _repl_green  = (sla_repl_days > 0 and bool(rpols_list)
                            and rep_n > 0 and dests_ok
                            and repl_ret_n >= sla_repl_days)
            _wb = _compute_row_color(_local_green, _repl_green,
                                     local_n > 0, rep_n > 0, sm_on)
            _ckey = f'{arr}|FB-Filesystem|{fs_name}'
            row_metas.append({
                'key':            _ckey,
                'array_name':     arr,
                'volume_name':    fs_name,
                'array_type':     'FB-Filesystem',
                'would_be_color': _wb})
            tr_html_t3 += (
                '<tr>'
                f'<td class="cde-name" data-ckey="{_ckey}">'
                f'{_html.escape(fs_name)}</td>'
                f'<td>{_html.escape(arr)}</td>'
                f'<td style="{_OK if dests_ok else _BAD}">{dests}</td>'
                f'<td style="{_OK if rfs_ok else _BAD}">{rfs_cell}</td>'
                f'<td style="{_OK if local_n > 0 else _BAD}text-align:right;">{local_n}</td>'
                f'{rep_cell}'
                f'<td style="{sm_style}">{sm_inner}</td>'
                f'<td style="{_OK if pols_ok  else _BAD}">{pols_cell}</td>'
    ...
    # Static info banner above Table 1 pointing readers at the
    # GUI-managed exceptions sidecar. Only rendered when there's at
    # least one volume row to annotate.
    if rows:
        comments_toolbar = (
            '<div class="cde-toolbar">'
            '<span class="cde-toolbar-hint">Config Drift Exceptions are '
            'managed from the desktop GUI \u2014 click '
            '"Manage Exceptions" under "Volume and Filesystem '
            'Protection Report" to edit '
            '<code>reports/protection/exceptions.json</code> and '
    ...
    _has_fb_exp = bool(fb_export_profiles)
    modal_block_parts = []
    if _has_pg:
    ...
    if _has_dir:
        modal_block_parts.append(
            ...
            '</div></div>')
    if _has_fb_exp:
        # FlashBlade Exports modal. Distinct from the FA export-modal
        # because the FB payload renders a Protocols mini-table plus
        # one Export Info / NFS Rules / SMB Rules section per export
        # row that shares the clicked name. The DOM nodes mirror the
        # FA export modal's ids with an "fb-" prefix so the renderer
        # never reaches across modals.
        modal_block_parts.append(
            '<div id="fb-export-modal" class="modal" '
            'onclick="closeModalIfBg(event,\'fb-export-modal\')">'
            '<div class="modal-content">'
            '<span class="modal-close" onclick="closeModalById(\'fb-export-modal\')">&times;</span>'
            '<h3 id="fb-export-title"></h3>'
            '<p class="meta" id="fb-export-meta"></p>'
            '<div id="fb-export-protocols"></div>'
            '<div id="fb-export-sections"></div>'
            '</div></div>')
    modal_block = '\n'.join(modal_block_parts)

    if _has_pg or _has_policy or _has_export or _has_dir or _has_fb_exp:
        script_parts = ['<script>\n']
        ...
        if _has_fb_exp:
            script_parts.append('const FB_EXPORT_PROFILES = '
                                + fb_export_profiles_json + ';\n')
        ...
        if _has_fb_exp:
            # FlashBlade export modal. Each clicked Exports name maps
            # to a FB_EXPORT_PROFILES[arr][name] entry carrying:
            #   protocols    - {protocols, access_control_style,
            #                   safeguard_acls} for the row's
            #                   filesystem from purefs list
            #                   --multi-protocol. Rendered as a single
            #                   mini-table at the top of the modal.
            #   exports      - one record per purefs export list row
            #                   sharing the clicked name (an export
            #                   with both NFS and SMB attachments
            ...
            script_parts.append(
                ...
                '["anongid","Anongid"],'
                '["secure","Secure"],'
                '["fileid_32bit","Fileid 32Bit"],'
                '["atime","Atime"],'
                '["index","Index"],'
                '["required_transport_security","Required Transport Security"]];\n'
                'const FB_SMB_RULE_COLS=['
                '["policy","Policy"],'
                '["principal","Principal"],'
                '["full_control","Full Control"],'
                '["change","Change"],'
                '["read","Read"]];\n'
                'const FB_EXPORT_COLS=['
                '["name","Name"],'
                ...
                '["enabled","Enabled"],'
                '["status","Status"]];\n'
                'function fbRenderKV(obj,cols){'
                'let h=\'<table class="pg-detail"><tbody>\';'
                'cols.forEach(c=>{h+=\'<tr><th>\'+escHtml(c[1])+\'</th><td>\''
                '+escHtml(obj[c[0]]||"")+\'</td></tr>\';});'
                'return h+\'</tbody></table>\';}\n'
                'function fbRenderRules(rules,cols){'
                'if(!rules||!rules.length)return "";'
                'let h=\'<table class="pg-detail"><thead><tr>\';'
                'cols.forEach(c=>{h+=\'<th>\'+escHtml(c[1])+\'</th>\';});'
                'h+=\'</tr></thead><tbody>\';'
                ...
                'h+=\'</tr>\';});'
                'return h+\'</tbody></table>\';}\n'
                'function showFbExport(el){'
                'const arr=el.getAttribute("data-arr"),en=el.getAttribute("data-exp");'
                'const data=(FB_EXPORT_PROFILES[arr]||{})[en]||null;'
                'document.getElementById("fb-export-title").textContent='
                '"Export Details - "+en;'
                'document.getElementById("fb-export-meta").textContent='
                '"Source array: "+arr+(data&&data.file_system?'
                '("  -  File System: "+data.file_system):"");'
                'let protoHtml="",secHtml="";'
                'if(!data){secHtml=\'<p style="color:#888;">No data.</p>\';}'
                'else{'
                'const p=data.protocols||{};'
                'if(p.protocols||p.access_control_style||p.safeguard_acls){'
                'protoHtml=\'<h4>Protocols</h4>\'+fbRenderKV(p,['
                '["protocols","Protocols"],'
                '["access_control_style","Access Control Style"],'
                '["safeguard_acls","Safeguard ACLs"]]);}'
                'const exps=data.exports||[];'
                'if(!exps.length){secHtml=\'<p style="color:#888;">'
                'No export rows.</p>\';}'
                'else{exps.forEach(e=>{'
                'const lbl=(e.type||"Export")+((e.share_policy)?'
                '(" - "+e.share_policy):"");'
                'secHtml+=\'<h4 style="margin-top:1em;">Export Info - \''
                '+escHtml(lbl)+\'</h4>\';'
                ...
                '});}}'
                'document.getElementById("fb-export-protocols").innerHTML=protoHtml;'
                'document.getElementById("fb-export-sections").innerHTML=secHtml;'
                'document.getElementById("fb-export-modal").style.display="flex";}\n')
    ...
    _T1_COLS = [
        ...
        'Connected Hosts', 'Config Drift Exceptions']
    ...
    _T2_COLS = [
        ...
        'Max Local Snap Retention (Days)', 'Max Repl Snap Retention (Days)',
        'Local Snap Retention vs SLA', 'Repl Snap Retention vs SLA',
        'Exports', 'Config Drift Exceptions']
    # Table 3 mirrors Table 2 without the Pod / Remote Pod columns
    # (FlashBlade filesystems are not pod-scoped). A "Replica File
    # System" column sits beside Replication Destination to surface
    # the destination-side filesystem name(s) for each outbound link;
    # destination-side rows themselves are suppressed by the aggregator
    # so the same filesystem never renders twice. The Protocols column
    # is derived from `purefs list --multi-protocol`; the Exports cell
    # is clickable and opens a per-export popup; the rightmost "Config
    # Drift Exceptions" column is populated from exceptions.json.
    _T3_COLS = [
        'Filesystem Name', 'Array Name', 'Replication Destination',
        'Replica File System', 'Local Snapshots', 'Replicated Snapshots',
        'Safemode', 'Snapshot Policies',
        'Max Local Snap Retention (Days)', 'Max Repl Snap Retention (Days)',
        'Local Snap Retention vs SLA', 'Repl Snap Retention vs SLA',
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    # Discovery pass: reconcile the rows captured above against
    # exceptions.json before applying the saved color / exception
    # reason / Last_Update to the rendered HTML.
    _discover_and_update_exceptions(row_metas)
...
<head>
  ...
  <style>
    ...
    /* First-column "name" cell shaded by exceptions.json color. Any
       <a> inside inherits the cell's foreground color so anchors
       don't override the row's red/green/grey state. */
    td.cde-name a {{ color: inherit; }}
    /* Config Drift Exceptions column: exception reason on top and the
       Last Updated timestamp directly underneath. Populated from
       reports/protection/exceptions.json by the GUI's "Manage
       Exceptions" dialog and re-applied to the HTML at report time. */
    td.cde-cell {{ min-width: 220px; padding: 4px; }}
    td.cde-cell .cde-text {{
        font: 9pt 'Segoe UI', Arial, sans-serif;
        white-space: pre-wrap; word-break: break-word; }}
    td.cde-cell .cde-meta {{
        margin-top: 3px; font-size: 8.5pt; color: #555;
        white-space: nowrap; overflow: hidden;
        text-overflow: ellipsis; }}
    td.cde-cell .cde-meta-lbl {{ color: #888; }}
    td.cde-cell .cde-updated {{ color: #1f3a5c; }}
    /* Banner above Table 1 pointing to the GUI dialog that maintains
       exceptions.json. */
    .cde-toolbar {{ margin: 6px 0 8px 0; }}
    .cde-toolbar .cde-toolbar-hint {{
        font-size: 9pt; color: #666; }}
    .cde-toolbar .cde-toolbar-hint code {{
...
"""
    # Apply exceptions.json overlay (per-row color + exception cell)
    # as the final post-processing step before returning the HTML.
